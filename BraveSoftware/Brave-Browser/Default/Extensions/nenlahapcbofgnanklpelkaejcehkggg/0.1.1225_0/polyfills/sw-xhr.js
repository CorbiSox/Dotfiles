// Manual import of sw-xhr npm package
function __extends(d, b) {
  for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; // eslint-disable-line no-prototype-builtins
  function __() {
    this.constructor = d;
  }
...
var XMLHttpRequestPolyfilleEvents = (function (_super) {
  ...
  XMLHttpRequestPolyfilleEvents.prototype.removeEventListener = function (type, listener, options) {
    switch (type) {
      case 'readystatechange':
        this.events.readystatechange = this.events.readystatechange.filter(function (e) {
          return e !== listener;
        });
        break;
      case 'abort':
        this.events.abort = this.events.abort.filter(function (e) {
          return e !== listener;
        });
        break;
      case 'error':
        this.events.error = this.events.error.filter(function (e) {
          return e !== listener;
        });
        break;
      case 'load':
        this.events.load = this.events.load.filter(function (e) {
          return e !== listener;
        });
        break;
      case 'loadend':
        this.events.loadend = this.events.loadend.filter(function (e) {
          return e !== listener;
        });
        break;
      case 'loadstart':
        this.events.loadstart = this.events.loadstart.filter(function (e) {
          return e !== listener;
        });
        break;
      case 'progress':
        this.events.progress = this.events.progress.filter(function (e) {
          return e !== listener;
        });
        break;
      case 'timeout':
        this.events.timeout = this.events.timeout.filter(function (e) {
          return e !== listener;
        });
        break;
      default:
...
self.XMLHttpRequest = (function (_super) {
...
// end Manual import of sw-xhr npm package
